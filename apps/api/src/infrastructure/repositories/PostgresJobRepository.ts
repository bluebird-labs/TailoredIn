...
} from '@tailoredin/domain-job';
import type {
  JobRepository,
  FindTopScoredParams,
  FindScoredParams,
  UpsertJobProps
} from '@tailoredin/application-job';
...
import { JobOrmRepository, type JobScoresProps } from '../db/entities/jobs/JobOrmRepository.js';
import { Company as OrmCompany } from '../db/entities/companies/Company.js';
import { Skill as OrmSkill } from '../db/entities/skills/Skill.js';
...
export class PostgresJobRepository implements JobRepository {
  ...
  private mapScores(ormScores: JobScoresProps): JobScores {
    const mapAffinity = (affinity: OrmSkillAffinity) => ({
      ...
      matches: ormScores.skills[affinity].matches.map(s => ({
        id: new SkillId(s.id),
        name: s.name,
        key: s.key,
        ...
        clearDomainEvents: () => {},
        refresh: () => {}
      } as unknown as import('@tailoredin/domain-job').Skill))
    });
    ...
    return {
      ...
      skills: {
        total: {
          ...
          matches: ormScores.skills.total.matches.map(s => ({
            id: new SkillId(s.id),
            name: s.name,
            key: s.key,
            affinity: s.affinity as unknown as SkillAffinity,

...
import { Archetype, SkillName } from '@tailoredin/domain-job';
import type {
  LlmService,
...
} from '@tailoredin/application-resume';
...
const archetypeDetails: Record<Archetype, string> = {
  ...
  [Archetype.IC]:
    'An engineer who is not expected to manage individuals and spends their whole time coding',
  [Archetype.LEAD_IC]:
    'An experienced engineer who demonstrates leadership and guides teams',
  [Archetype.NERD]:
    'A highly technical engineer focused on deep, cutting-edge technical work or research'
};
      ...
      messages: [
        {
          ...
          content:
            ...
            'Archetypes:\n' +
            Object.entries(archetypeDetails).map(([k, v]) => `- ${k}: ${v}`).join('\n')
        },

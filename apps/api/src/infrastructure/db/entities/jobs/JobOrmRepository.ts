...
const SKILL_LIST_KEYS = ['expert_skills', 'interest_skills', 'avoid_skills'] as const satisfies (keyof IFindTopScoredJobsResult)[];
...
export class JobOrmRepository extends BaseRepository<Job> {
  public async findTopScored<Hint extends string = never>(
    ...
    const [jobs, skills] = await Promise.all([
      ...
      this.getEm(opts).repo(Job).find({ id: { $in: jobIds } }, opts),
      // biome-ignore lint/style/useNamingConvention: MikroORM filter operator
      this.getEm(opts).repo(Skill).find({ id: { $in: Array.from(skillIds) } })
    ]);
  ...
  public async findScoredByIdOrFail<Hint extends string = never>(
    ...
    // biome-ignore lint/style/useNamingConvention: MikroORM filter operator
    const skills = await this.getEm(opts).repo(Skill).find({ id: { $in: Array.from(skillIds) } });
    const skillMap = new Map(skills.map(s => [s.id, s]));
    const scores = this.buildScores(row, skillMap);

...
import type { ScrapeAndIngestJobs } from '@tailoredin/application-job';
...
const searchAll = async () => {
  const useCase = container.get(ApplicationJobDI.ScrapeAndIngestJobs) as ScrapeAndIngestJobs;
  const searches = Object.entries(config.searches);
  const activeSearches = config.only
    ? searches.filter(([name]) => config.only?.includes(name))
    : searches;

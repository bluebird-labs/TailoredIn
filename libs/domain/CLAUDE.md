...
**`libs/domain/DOMAIN.mmd` is the source of truth for the domain model.** Whenever you add, remove, or rename an aggregate, entity, value object, enum, domain service, or domain event — update `DOMAIN.mmd` in the same commit. The diagram must always reflect the code.
...
Interfaces only — no implementation here. Live in `libs/domain/src/ports/`:

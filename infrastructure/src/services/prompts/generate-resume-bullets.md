...
- **Bullet count:** Generate between `minBullets` and `maxBullets` bullets for each experience — as specified per experience below. Aim for the midpoint of the range. Only approach `maxBullets` when the experience has exceptionally rich accomplishment data with distinct, high-impact achievements. Prefer fewer, stronger bullets over more, weaker ones.

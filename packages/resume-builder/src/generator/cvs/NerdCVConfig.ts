...
import { JobTitle }     from '../data/types';
import { Lantern }      from '../data/Lantern';
import { Brightflow }   from '../data/Brightflow';
import { Volvo }        from '../data/Volvo';
import { Luxe }         from '../data/Luxe';
import { StreamNation } from '../data/StreamNation';
import { Planorama }    from '../data/Planorama';
import { LuckyCart }    from '../data/LuckyCart';
...
export const NerdCVConfig: ResumeConfig = {
  ...
  experience: [
    ...
    Brightflow.position({
      ...
      highlights: [
        'Built an event-driven ETL framework with Node.js, Kafka (MKS), and Kubernetes Jobs (EKS), prioritizing robustness, scalability, and observability (X-Ray, CloudWatch, OpenTelemetry, Grafana)',
        'Delivered data to Athena (Apache Presto, S3-indexed) for analytics/ML, Aurora (Postgres) through REST APIs for real-time applications, and complex Excel reports for loan underwriting',
    ...
    LuckyCart.position({
      ...
      highlights: [
        'Maintained and scaled the 5 microservices Node / Java architecture under the CTO’s guidance',

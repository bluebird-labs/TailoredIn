...
import { TransientSkillCreateProps } from '../entities/skills/TransientSkill.types';
import { SkillAffinity } from '../entities/skills/SkillAffinity';
...
export class SkillsSeeder extends Seeder {
  public async run(em: EntityManager): Promise<void> {
    const transientSkills: TransientSkill[] = [];

    for (const [name, { affinity, variants }] of skills.entries()) {
      transientSkills.push(
        TransientSkill.create({
          name: name,
          affinity: affinity,
          variants: variants
        })
    ...
    const stats = await em.repo(Skill).refreshAll(transientSkills, { em });

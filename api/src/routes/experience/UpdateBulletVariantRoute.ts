import { inject, injectable } from '@needle-di/core';
import type { UpdateBulletVariant } from '@tailoredin/application';
import { DI } from '@tailoredin/infrastructure';
import { Elysia, t } from 'elysia';

@injectable()
export class UpdateBulletVariantRoute {
  public constructor(private readonly updateVariant: UpdateBulletVariant = inject(DI.Experience.UpdateVariant)) {}

  public plugin() {
    return new Elysia().put(
      '/variants/:id',
      async ({ params, body, set }) => {
        const result = await this.updateVariant.execute({
          experienceId: body.experience_id,
          bulletId: body.bullet_id,
          variantId: params.id,
          text: body.text,
          angle: body.angle,
          roleTags: body.role_tags ?? [],
          skillTags: body.skill_tags ?? []
        });
        if (!result.isOk) {
          set.status = 404;
          return { error: { code: 'NOT_FOUND', message: result.error.message } };
        }
        return;
      },
      {
        params: t.Object({ id: t.String({ format: 'uuid' }) }),
        body: t.Object({
          experience_id: t.String({ format: 'uuid' }),
          bullet_id: t.String({ format: 'uuid' }),
          text: t.String({ minLength: 1 }),
          angle: t.String({ minLength: 1 }),
          role_tags: t.Optional(t.Array(t.String())),
          skill_tags: t.Optional(t.Array(t.String()))
        })
      }
    );
  }
}
